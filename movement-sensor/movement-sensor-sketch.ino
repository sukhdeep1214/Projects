...
const unsigned long MOVEMENT_COOLDOWN = 5000;
...
void setup()
{
  ...
  Serial.println("UNO Q LSM6DSOX Motion Detector");
  ...
  // Get initial acceleration values
  uint8_t a[6];

  if (imuRead(ACCEL_REG, a, 6))
  {
    oldAx = (int16_t)((a[1] << 8) | a[0]);
    oldAy = (int16_t)((a[3] << 8) | a[2]);
    oldAz = (int16_t)((a[5] << 8) | a[4]);
  }
...
void loop()
{
  if (movementDetected())
  {
    if (millis() - lastMovement >= MOVEMENT_COOLDOWN)
    {
      Serial.println("MOTION");

      // Send motion event to Python/Linux
      Bridge.call("motion_detected");
